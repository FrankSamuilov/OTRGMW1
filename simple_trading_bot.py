...
class SimpleTradingBot:
    def __init__(self, client=None, config=None):
        ...
        try:
            ...
            self.logger.error(f"无法导入止损组件: {e}")
            self.smart_trailing_stop = None
            self.atr_stop_loss = None
    ...
    def _initialize_game_theory_components(self):
        ...
        try:
            ...
            self.data_collector = MarketDataCollector(self.client)
            ...
            self.logger.info("✅ 博弈论系统初始化成功")
            print_colored("✅ 博弈论系统初始化成功", Colors.GREEN)
    ...
    def _test_connection(self):
        ...
        try:
            ...
            if time_diff > 5000:  # 5秒以上的时间差
                print_colored(f"⚠️ 本地时间与服务器时间相差 {time_diff / 1000:.1f} 秒", Colors.WARNING)
                self.logger.warning(f"时间差过大: {time_diff}ms")

            # 测试账户访问
            account = self.client.futures_account()

            print_colored("✅ API连接测试成功", Colors.GREEN)
            self.logger.info("API连接正常")
            return True

        except Exception as e:
            print_colored(f"❌ API连接测试失败: {e}", Colors.ERROR)
        ...
        try:
            ...
            if os.path.exists(history_file):
                with open(history_file, 'r') as f:
                    ...
                    self.logger.info(f"加载了 {len(self.position_history)} 条历史持仓记录")
            else:
                self.position_history = []
                self.logger.info("未找到历史持仓文件，创建新的记录")

        except Exception as e:
            self.logger.error(f"加载持仓历史失败: {e}")
            self.position_history = []
